...
public partial class BaseAa : BaseNPC
{
    ...
    public virtual void AA_MoveTo(dest, playAnim, moveType, extraOptions)
    {
        ...
        if (debug)
        print("----------------");
        print("[MoveTo] My WaterLevel: " + WaterLevel);
        ...
        if (WaterLevel <= 2) self:MaintainIdleBehavior(1) return 
        ...
        start = dest:GetPos() + Collider.Bounds.Center,;
        endpos = (dest:GetPos() + Collider.Bounds.Center) + dest:GetUp()*-20,;
        filter = trFilter;
        });
        //PrintTable(trene)
        ...
        var startPos = myPos + Collider.Bounds.Center + vecStart // nearpos.MyPosition;
