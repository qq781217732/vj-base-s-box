...
public partial class PlayerBase
{
	...
	bool IsDuckHeld()
	{
		return !IsUsingController && !IsDuckSuppressed() && Input.Down( InputButtonHelper.Duck );
	}

	bool IsDuckTogglePressed()
	{
		return Input.Pressed( InputButtonHelper.DuckToggle ) || (IsUsingController && Input.Pressed( InputButtonHelper.Duck ));
	}

	bool WantsToCrouch()
	{
		return IsDuckHeld() || stickyActiveButtons.Contains( InputButtonHelper.Duck );
	}

	bool WantsToEnterLowProfileThisFrame()
	{
		return Input.Pressed( InputButtonHelper.Duck ) || IsDuckTogglePressed();
	}

	bool HandleAimSprintInteraction( bool wantsToRun )
	{
		if ( !wantsToRun )
			return false;

		if ( Inventory?.ActiveItem is not SWB.Base.Weapon activeWeapon )
			return true;

		if ( activeWeapon.CanEnterAds() )
		{
			stickyActiveButtons.Remove( InputButtonHelper.Run );
			return false;
		}

		if ( activeWeapon.AdsCancelOnSprintStart && activeWeapon.IsAiming )
			activeWeapon.CancelAim();

		return true;
	}

	void SetDuckToggleActive( bool isActive )
	{
		if ( isActive )
	...
	void EnterCrouchState( bool clampHorizontalSpeed = false )
	{
		IsRunning = false;
		stickyActiveButtons.Remove( InputButtonHelper.Run );
		IsCrouching = true;
		...
		if ( clampHorizontalSpeed && CharacterController is not null )
		{
			var verticalSpeed = CharacterController.Velocity.z;
			var horizontalVelocity = CharacterController.Velocity.WithZ( 0 ).ClampLength( CrouchSpeed );
			CharacterController.Velocity = horizontalVelocity.WithZ( verticalSpeed );
		}
	}
	...
	public virtual void OnMovementUpdate()
	{
		...
		if ( !IsOnGround )
		{
			...
			LastInAirTime = TimeSinceBecameAirborne;
		...
		UpdateMovementFeelContinuousState();
		RotateBody();
		UpdateAnimations();
	}
	...
	public virtual void OnMovementFixedUpdate()
	{
		...
		CurrentMovementState =
			...
			IsClimbingLadder ? MovementState.Ladder :
	...
	void BuildWishVelocity()
	{
		...
		if ( IsSliding ) WishVelocity *= RunSpeed;
		else if ( IsCrouching ) WishVelocity *= CrouchSpeed;
	...
	void Move()
	{
		...
		if ( IsOnGround )
		{
			...
			{
				ApplyPostSlideCarry();
				carryActive = HasPostSlideCarry();
			}

			if ( !carryActive && !skipFrictionForPushOff )
			{
				CharacterController.Accelerate( WishVelocity );
				CharacterController.ApplyFriction( GroundControl );
			}
			skipFrictionForPushOff = false;
		}
		else
		{
			// Air control / Gravity
		...
		// Push player off NPC heads — only when standing on top, not just near
		if ( IsOnGround )
		{
			var traceDepth = CharacterController.Height * 0.5f + 8f;
			var groundTr = Scene.Trace.Ray( WorldPosition, WorldPosition + Vector3.Down * traceDepth )
				.WithoutTags( TagsHelper.Trigger )
				.IgnoreGameObjectHierarchy( GameObject )
				.Radius( 4 )
				.Run();

			// Only trigger if ground is an NPC AND we're on top of it (not beside it)
			var isNpc = groundTr.Hit && (groundTr.GameObject.Tags.Has( "npc" ) || groundTr.GameObject.Tags.Has( "zombie" ));
			var feetZ = WorldPosition.z - CharacterController.Height * 0.5f;
			var isOnTop = groundTr.HitPosition.z > groundTr.GameObject.WorldPosition.z + 32f;

			if ( isNpc && isOnTop )
			{
				// Standing on a zombie head — find best escape direction from all nearby
				var separationRadius = CharacterController.Radius + 32f;
				var nearby = Scene.FindInPhysics( new Sphere( WorldPosition, separationRadius ) );
				var pushSum = Vector3.Zero;

				foreach ( var obj in nearby )
				{
					if ( !obj.IsValid() || obj == GameObject ) continue;
					if ( !obj.Tags.Has( "npc" ) && !obj.Tags.Has( "zombie" ) ) continue;

					var toObj = WorldPosition.WithZ( 0 ) - obj.WorldPosition.WithZ( 0 );
					var dist = toObj.Length;
					if ( dist < 0.01f ) continue;

					pushSum += toObj.Normal * (1f - dist / separationRadius);
				}

				if ( !pushSum.IsNearZeroLength )
				{
					var pushVelocity = pushSum.Normal * 100f;
					if ( pushVelocity.Length > 150f )
						pushVelocity = pushVelocity.Normal * 150f;
					CharacterController.Velocity += pushVelocity;
					skipFrictionForPushOff = true;
				}
			}
		}
	...
	void Jump()
	{
		if ( !MovementFeelMath.CanUseGroundOrCoyoteJump( IsOnGround, TimeSinceLeftGround, JumpCoyoteTimeThreshold ) )
			return;
	...
	public virtual void OnGrounded( Vector3 velocity )
	{
		...
		if ( !IsProxy )
		{
			SetLandingState( velocity );

			var landingShake = MathF.Max( PendingLandingFeedback - 0.2f, 0f );
			if ( landingShake > 0f )
			{
				ShakeScreen( new()
				{
					Size = 0.08f + (landingShake * 0.2f),
					Rotation = 0.08f + (landingShake * 0.2f),
					Duration = 0.06f + (landingShake * 0.06f),
				} );
			}
	...
	void UpdateAnimations()
	{
		...
		AnimationHelper.WithWishVelocity( GetAnimationWishVelocity() );
		...
		AnimationHelper.DuckLevel = IsSliding ? 1f : GetCrouchBlend();
	}
	...
	void UpdateRun()
	{
		...
		var wantsToRun = runIsDownOrPressed || runIsStickyActive;
		wantsToRun = HandleAimSprintInteraction( wantsToRun );

		IsRunning = !IsSliding && !IsCrouching && wantsToRun;
	}

	void UpdateCrouch()
	{
		UpdateDuckSuppressUntilRelease();

		if ( IsDuckTogglePressed() )
			SetDuckToggleActive( !stickyActiveButtons.Contains( InputButtonHelper.Duck ) );

		if ( IsSliding )
			return;

		var wantsLowProfileThisFrame = WantsToEnterLowProfileThisFrame();
		if ( wantsLowProfileThisFrame && CanStartSlide() )
		{
			StartSlide();
			return;
		}

		UpdatePostSlideCrouchLock();
		var wantsToCrouch = WantsToCrouch() || ShouldPreserveCrouchAfterSlide();
		var shouldAutoCrouchFromSlideAttempt = wantsLowProfileThisFrame && AutoCrouchWhenSlideUnavailable;
		var shouldStayLow = wantsToCrouch || shouldAutoCrouchFromSlideAttempt;
		if ( !IsCrouching && shouldStayLow && CrouchSpamPrevention && TimeSinceCrouch < 0.5f )
			return;

		if ( !IsCrouching && shouldStayLow && IsOnGround && !IsClimbingLadder )
			EnterCrouchState( false );  // Never clamp speed when entering crouch; preserve momentum

		if ( IsCrouching && !wantsToCrouch )
		{
			if ( CanStandAtHeight( GetTargetStandingHeight() ) )
			{
				IsCrouching = false;
