...
public class CameraMovement : Component, ICameraMovement
{
	...
	protected override void OnUpdate()
	{
		...
		Player.FieldOfView = Screen.CreateVerticalFieldOfView( Preferences.FieldOfView + Player.CameraFovAdditive );
	...
	public virtual void FirstPerson()
	{
		...
		var targetOffset = Vector3.Down * Player.FirstPersonPostureOffset;

		var firstPersonCameraVerticalOffset = Player.FirstPersonCameraVerticalOffset;
		var firstPersonCameraPitchOffset = Player.FirstPersonCameraPitchOffset;

		Player.EyeOffset = Vector3.Lerp( Player.EyeOffset, targetOffset, Time.Delta * 10f );

		eyePos += PosOffset;
		eyePos += Vector3.Down * firstPersonCameraVerticalOffset;
		PosOffset = Vector3.Zero;

		eyeAngles += AnglesOffset;
		eyeAngles += new Angles( firstPersonCameraPitchOffset, 0f, 0f );
