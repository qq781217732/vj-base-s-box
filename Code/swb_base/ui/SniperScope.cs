...
public class SniperScope : Panel
{
	...
	public SniperScope( Weapon weapon, string lensTexture, string scopeTexture )
	{
		...
		if ( scopeTexture is not null )
			AddChild<Panel>().AddClass( "leftBar" );

		lensWrapper = AddChild<Panel>();
		lensWrapper.AddClass( "lensWrapper" );

		var lensImage = lensWrapper.AddChild<Image>();
		if ( lensTexture is not null ) lensImage.Texture = Texture.Load( lensTexture );
		lensImage.AddClass( "lens" );

		if ( scopeTexture is not null )
		{
			scope = lensWrapper.AddChild<Image>();
			scope.Texture = Texture.Load( scopeTexture );
			scope.AddClass( "scope" );

			AddChild<Panel>().AddClass( "rightBar" );
			AddChild<Panel>().AddClass( "topBar" );
			AddChild<Panel>().AddClass( "bottomBar" );
			AddChild<Panel>().AddClass( "leftBarOffscreen" );
			AddChild<Panel>().AddClass( "rightBarOffscreen" );
		}
	}
