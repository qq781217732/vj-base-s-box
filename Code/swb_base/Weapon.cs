...
public partial class Weapon : Component, IInventoryItem
{
	...
	protected override void OnUpdate()
	{
		...
		if ( !IsProxy && !Owner.IsBot && !IsDeploying )
		{
			...
			var shouldAim = IsAiming
				? IsAimInputActive() && !ShouldCancelAds()
				: CanEnterAds();

			SetAimState( shouldAim );
	...
	void SetAimState( bool shouldAim )
	{
	...
	public bool IsAimInputActive()
	{
		if ( AimAnimData == AngPos.Zero )
			return false;

		var isAimInputDown = Input.Down( InputButtonHelper.SecondaryAttack );
		if ( !isAimInputDown )
		{
			suppressAimUntilInputReleased = false;
			return false;
		}

		return !suppressAimUntilInputReleased;
	}

	public bool CanEnterAds()
	{
		if ( !IsAimInputActive() )
			return false;

		if ( AdsBlockWhileCustomizing && IsCustomizing )
			return false;

		if ( AdsBlockWhileReloading && IsReloading )
			return false;

		if ( AdsBlockWhileRunning && IsRunning )
			return false;

		if ( AdsBlockWhileSliding && Owner.IsSliding )
			return false;

		return true;
	}

	public bool ShouldCancelAds()
	{
		if ( AdsBlockWhileCustomizing && IsCustomizing )
			return true;
	...
	public void CancelAim( bool suppressUntilRelease = true )
	{
		if ( suppressUntilRelease && Input.Down( InputButtonHelper.SecondaryAttack ) )
			suppressAimUntilInputReleased = true;
