...
public class PlayerCameraHandler : Component
{
	...
	protected override void OnDestroy()
	{
		...
		if ( player is SWB.Player.PlayerBase movementPlayer )
		{
			movementPlayer.SetZoomState( false, false );
			movementPlayer.SetCameraFovAdditive( 0f );
		}
	}
	...
	protected override void OnUpdate()
	{
		...
		var isZooming = !Weapon.ShouldTuckVar && Weapon.IsAiming && !Weapon.IsReloading;
		var isScopedZoom = isZooming && Weapon.IsScoping;
		if ( player is SWB.Player.PlayerBase movementPlayer )
		{
			movementPlayer.SetZoomState( isZooming, isScopedZoom );
			movementPlayer.SetCameraFovAdditive( finalPlayerFOV - Preferences.FieldOfView );
		}
