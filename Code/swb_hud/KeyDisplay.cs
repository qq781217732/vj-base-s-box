...
public class KeyDisplay : Panel
{
	...
	void CreateKeys()
	{
		...
		{
			var wrapper = AddChild<Panel>();
			wrapper.AddClass( "wrapper" );
			var keyIcon = wrapper.AddChild<Image>();
			keyIcon.AddClass( "keyIcon" );
			var buttonTexture = Input.GetGlyph( keyBind.Key, style: GlyphStyle.Dark );
			keyIcon.Texture = buttonTexture;

			var keyLabel = wrapper.AddChild<Label>();
			keyLabel.Text = keyBind.Label;
			keyLabel.AddClass( "label" );
		} );
	}
